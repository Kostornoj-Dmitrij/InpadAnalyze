    ...
    messages = [
        ...
        },
        {
            ...
            "content": (
                ...
                "- Нарушение 1: [Описание] (Норматив: XXX, ТЗ стр.X / РР стр.Y)\n\n"
...
async def generate_answer(file_path: str, question: str) -> str:
    try:
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": f"Контекст:\n{'\n\n'.join(context)}\n\nВопрос: {question}. ОБЯЗАТЕЛЬНО УКАЖИ СТРАНИЦУ НА КОТОРОЙ НАШЁЛ ОТВЕТ. Если ты не нашёл ответ в этом фрагменте, то ни в коем случае не указывай страницы, чтобы не выглядело так: в предоставленном фрагменте нет такой информации. [Страницы 23-29] "
            }
        ]
        retries = 3
        while retries > 0:
